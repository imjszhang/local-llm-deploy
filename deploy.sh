...
if [ "$HOST" = "0.0.0.0" ]; then
    ...
    echo -e "${GREEN}后端:     http://${HOST}:$PORT (仅本机)${NC}"
fi
echo ""
echo -e "${YELLOW}前端代理（含监控 + 推理队列）需单独启动:${NC}"
echo "   ./serve-ui.sh"
echo "   然后访问: http://localhost:8888/monitor.html"
echo ""
echo -e "${GREEN}OpenAI 兼容接口 (通过代理，推荐):${NC}"
UI_PORT="${UI_PORT:-8888}"
echo "   http://localhost:${UI_PORT}/api/${PID_FILE_NAME}/v1"
[ -n "$LAN_IP" ] && echo "   http://${LAN_IP}:${UI_PORT}/api/${PID_FILE_NAME}/v1"
echo -e "${GREEN}OpenAI 兼容接口 (直连后端，无队列保护):${NC}"
