...
from ..config import ConfigError, load_apps, load_env_file, load_proxies, load_specs, project_paths
...
def main(argv=None, *, paths=None):
    ...
    try:
        ...
        apps = load_apps(paths.registry) if paths.registry.exists() else {}
        if args.model_name in apps or any(spec.alias == args.model_name for spec in apps.values()):
            raise ConfigError(f"{args.model_name} 是应用入口，不能下载")
