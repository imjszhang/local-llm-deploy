...
class GatewayContext:
    def __init__(self, paths=None, *, settings=None, specs=None, discovery=None,
                 ...
                 spec_loader=None, registry_ttl=30, clock=time.monotonic):
    ...
    def resolve(self, path, method, body, headers):
        with self._catalog_lock:
            self.refresh()
            router, models = self.router, self.discovery.models()
...
class ProxyHandler(SimpleHTTPRequestHandler):
    ...
    def handle_expect_100(self):
        ...
        try:
            ...
            if path.startswith(('/api/', '/v1/')) and path not in ('/api/models', '/api/system'):
    ...
    def _json(self, writer, payload):
        writer.start(200, {'Content-Type': 'application/json'})
