...
const shared = inject(consoleContext, null);
const monitor = shared?.monitor ?? useMonitor();
const {
  snapshot,
  detail,
  publicOverview,
        ...
        >
          <svg viewBox="0 0 20 20" fill="none" aria-hidden="true">
            ...
            <path
              ...
              stroke-width="1.4"
            /></svg
          ><span>{{ shared?.credentialSource?.value === 'local' ? '本机已连接' : '访问设置' }}</span
          ><span
            v-if="keySet"
