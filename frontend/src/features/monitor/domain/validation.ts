...
const chatControls = (v: unknown): ChatControls | null => {
  // Older gateways have no capability declaration; never infer controls from a model name.
  if (v === undefined || v === null) return null
  const o = object(v)
  const efforts = array(o.reasoning_efforts, value => {
    const effort = string(value)
    return /^[a-z][a-z0-9_-]{0,23}$/.test(effort) && effort !== 'none' ? effort : fail()
  }, 8)
  const thinking = boolean(o.thinking), budget = boolean(o.reasoning_budget)
  const defaultThinking = nullableBoolean(o.default_thinking), defaultEffort = nullableString(o.default_effort)
  if (new Set(efforts).size !== efforts.length || (defaultEffort !== null && !efforts.includes(defaultEffort))) return fail()
  if (!thinking && (efforts.length || budget || defaultThinking === true || defaultEffort !== null)) return fail()
  return { thinking, reasoning_efforts: efforts, reasoning_budget: budget,
    default_thinking: defaultThinking, default_effort: defaultEffort, source: oneOf(o.source, ['configured']) }
}
const model = (v: unknown): MonitorModel => {
  ...
  return {
    ...
    chat_controls: chatControls(o.chat_controls),
