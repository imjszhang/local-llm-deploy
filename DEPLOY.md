...
控制层支持 Python 3.9+；目前本机验证包含 Python 3.9.6 和 3.14.4。MLX 后端需 Apple Silicon。注册表和权重目录可沿用已有部署。
...
下面列出的约束来自已安装依赖的传递闭包，版本验证结果见 [validation.md](docs/validation.md)。Python 次版本和硬件平台必须匹配，其他组合应创建自己的约束基线。
...
实际解释器可使用完整路径；已有虚拟环境不要重复创建，升级采用新目录，见 [升级指南](docs/upgrade.md)。Whisper 的启动描述会将项目 `tools/` 放入 PATH，`tools/ffmpeg` 使用安装的 imageio-ffmpeg。

也可通过兼容依赖文件安装，例如 `pip install -r requirements-embedding.txt`。这些入口引用相应 extra；需要锁定版本时同时加 `-c constraints/...`。
